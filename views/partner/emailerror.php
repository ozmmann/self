<?php
$this->title = 'Письмо не отправлено';
?>
<div id="success_page">
    <div class="container">
        <div id="info_page_wrapper" class="w-85 m-auto text-center">
            <img src="/img/success-image.png" class="img-response mtop-40 mtop-lg-80">
            <div class="f-34 lh-1-4 fw-semi-bold mtop-40">Письмо не отправлено<br>
            </div>
            <div class="f-14 c-gray lh-1-4 fw-light mtop-40">
                Обратитесь к <a href="email:<?= Yii::$app->params['adminEmail'] ?>">администратору</a>
            </div>
        </div>
    </div>
</div>